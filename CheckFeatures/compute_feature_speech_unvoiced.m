...
function feature_for_unvoiced_frames = compute_feature_speech_unvoiced (audio_file, feature_number)
    ...
    feature_for_unvoiced_frames = [];
    for i = 1:length(final_label_array)
        if (final_label_array(i) == 0)        
            feature_for_unvoiced_frames = [feature_for_unvoiced_frames; feature(i)];     
        end    
    end
