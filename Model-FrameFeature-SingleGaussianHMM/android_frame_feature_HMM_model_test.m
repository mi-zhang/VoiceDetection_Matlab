...
function  android_frame_feature_HMM_model_test (audio_file, number_of_features)
    ...
    % save the results
    fName = 'android_performance_single_gaussian_HMM.txt';         
    fid = fopen(fName, 'a');            
    if fid ~= -1
        fprintf(fid,'%s\r\n', audio_file);  
%         fprintf(fid,'%s\r\n', 'TPR, FPR, Accuracy, Precision, Recall');
    ...
    % save the plot
    saveas(gcf, [audio_file '_frame_feature_HMM_model_android'], 'jpg');
