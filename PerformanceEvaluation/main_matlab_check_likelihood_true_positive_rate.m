...
function main_matlab_check_likelihood_true_positive_rate (audio_file)
...
%     voiced_liklihood_threshold_array = [-0.5 -1 -1.5 -2 -2.5 -3 -3.5 -4 -4.5 -5];
    TPR_array = zeros(size(voiced_liklihood_threshold_array));
    ...
    for j = 1:length(voiced_liklihood_threshold_array)
        ...
        % filter out true positive based on voiced_liklihood_threshold
    ...
    % plot the TPR curve
    figure;
    plot(voiced_liklihood_threshold_array, TPR_array, 'rs-');
    title([audio_file, ': Voiced Likelihood'], 'FontSize', 18);
    xlabel('Voiced Likelihood Threshold (log10)');
    ylabel('True Positive Rate (%)');

    % save the TPR curve
    saveas(gcf, [audio_file '_voiced_likelihood_TPR'], 'jpg');    
