...
function main_matlab_check_peak_number_false_positive_rate (audio_file)
    ...
    FPR_array = zeros(size(peak_number_threshold_array));
    ...
    % plot the FPR curve
    figure;
    plot(peak_number_threshold_array, FPR_array, 'rs-');
    title([audio_file, ': Peak Number'], 'FontSize', 18);
    xlabel('Peak Number Threshold');
    ylabel('False Positive Rate (%)');

    % save the FPR curve
    saveas(gcf, [audio_file '_peak_number_FPR'], 'jpg');    
