function [transmat, obsmat, exp_num_trans, exp_num_emit, gamma, ll] = online_em(...
...
% E step
olikseq = mk_dhmm_obs_lik(data, obsmat);
if isempty(act)
  [gamma, xi, ll] = forwards_backwards(prior, transmat, olikseq, filter_only);
else
  [gamma, xi, ll] = forwards_backwards_pomdp(prior, transmat, olikseq, act, filter_only);
end
